...
module.exports = function(options) {
  ...
  return function(Bookshelf) {
    ...
    var fetchPage = modelProto.fetchPage;
    ...
    var Model = Bookshelf.Model.extend({
       ...
       * @memberof module:bookshelf-advanced-serialization.Model
       * @instance
       * @desc A boolean value to mark _accessor as frozen. You should not set this value directly; it is
       * set when creating a new model instance, or by `setAccessor()`.
       * @private
       */
      _accessorFrozen: false,
       ...
       */
      setAccessor: function(accessor, options) {
        this._accessor = accessor;
        if (options) {
          if (typeof options.freezeAccessor !== 'undefined') {
            this._accessorFrozen = options.freezeAccessor;
          }
        }
      },
      ...
      initialize: function() {
        modelProto.initialize.apply(this, arguments);
        var options = arguments[1] || {};
        this._accessor = options.accessor;
        if (typeof options.freezeAccessor !== 'undefined') {
          this._accessorFrozen = options.freezeAccessor;
       ...
       * @memberof module:bookshelf-advanced-serialization.Model
       * @instance
       * @desc Override the prototype's `fetchPage` so that this._accessor is set on all
       * models in a collection fetched by `fetchPage`.
       */
      fetchPage: function() {
        var modelThis = this;
        var result = fetchPage.apply(this, arguments);
        return result.then(function(pagination_results) {
          // http://bookshelfjs.org/#Model-instance-fetchPage
          if(pagination_results) {
            // Collection
            if (pagination_results.hasOwnProperty('models')) {
              _.each(pagination_results.models, function(model) {
                model._accessor = modelThis._accessor;
              }.bind(modelThis));

            // Model
            } else {
              result._accessor = modelThis._accessor;
            }
          }
        ...
        if (result) {
          ...
          if (result.hasOwnProperty('models')) {
            _.each(result.models, function(model) {
              // if accessor is akready frozen, it might have been forcibly set. Don't reset it
              if (!model._accessorFrozen) {
